...
//   const Products = [
//     { id: 1, name: "Men's Shirt", price: 70, img: p1 },
...
//     navigate("/checkoutpage", { state: { buyNowProduct: selectedProduct } });
//   };
//   return (
...
//                 <Col key={product.id} lg={4} md={4}>
//                   <div className="product-box">
//                     <div className="product-container">
//                       <div className="product-option">
//                         <Button onClick={() => dispatch(addToCart(product))} className='option1'>Add To Cart</Button>
//                         <Button className='option2' onClick={() => handleBuyNow(product)}>Buy Now</Button>
//                       </div>
//                     </div>
 ...
 const fetchCarouselData = async () => {
    try {
      ...
      if (res.data.length > 0) {
        setBackgroundImage(res.data[0].image); // Set initial background
      }
    } catch (err) {
      console.error("❌ Error fetching carousel data:", err);
    }
  ...
  const handleSelect = (selectedIndex) => {
    setIndex(selectedIndex);
      setBackgroundImage(carouselItems[selectedIndex]?.image);
  };
  const handleScrollToTop = (smoothScroll = false) => {
    window.scrollTo({
      top: 0,
  ...
  return (
    ...
    <div className="slider-section">
      <div className="slidebg">
        {backgroundImage && <img src={`http://localhost:4000${backgroundImage}`} alt="sliderbg" className="slider-image" />}
      </div>
      <Container fluid>
        <Row>
          <Col lg={12} xl={12}>
      ...
      <div className="custom-indicators">
        <ol className="carousel-indicators">
          {carouselItems.map((_, idx) => (
            <li key={idx} className={index === idx ? "active" : ""} onClick={() => handleSelect(idx)}></li>
          ))}
        </ol>
      </div>
    </div>
